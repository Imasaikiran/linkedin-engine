...
function buildSystemPrompt(brand: Brand): string {
  ...
  return [
    ...
    mustHave,
    '',
    'RHYTHM (these are HARD CONSTRAINTS — count before you submit):',
    `  - hook_max_words: ${rhythm.hook_max_words}. First line MUST be ≤${rhythm.hook_max_words} words. No trailing period. Count the words.`,
    `  - paragraph_max_lines: ${rhythm.paragraph_max_lines}. Insert a blank line whenever a paragraph would exceed ${rhythm.paragraph_max_lines} lines.`,
    `  - target_words: [${rhythm.target_words[0]}, ${rhythm.target_words[1]}] inclusive. Aim for the middle (~${Math.round((rhythm.target_words[0] + rhythm.target_words[1]) / 2)} words).`,
    `  - target_chars: [${rhythm.target_chars[0]}, ${rhythm.target_chars[1]}] inclusive.`,
    '',
    'STRUCTURE BY PILLAR:',
    '  - pillar="framework" or "list": numbered/bulleted lists are REQUIRED.',
    '  - All other pillars (shipped, story, lesson, critique, etc.): write as PROSE PARAGRAPHS. Do NOT use numbered lists, bullet points, or markdown list markers ("1.", "-", "*").',
    '',
